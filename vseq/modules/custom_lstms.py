...
Source: https://github.com/pytorch/pytorch/blob/master/benchmarks/fastrnns/custom_lstms.py
...
class LSTMCell(jit.ScriptModule):
    def __init__(self, input_size: int, hidden_size: int):
        """Regular LSTM cell [TUTORIAL USE ONLY]"""
...
class LayerNorm(jit.ScriptModule):
    def __init__(self, normalized_shape: Union[int, Iterable[int]]):
        """Fast Layer Normalization implementation c.f. [1].

        [1] http://arxiv.org/abs/1607.06450
        """
...
class LayerNormLSTMCell(jit.ScriptModule):
    def __init__(self, input_size, hidden_size, decompose_layernorm=False):
        """Fast Layer Normalized LSTM cell c.f. [1]
...
if __name__ == "__main__":
    ...
    seq_len, batch, input_size, hidden_size, num_layers = 5, 2, 3, 7, 4
    inp = torch.randn(seq_len, batch, input_size).to(device)
    h_0 = torch.randn(batch, hidden_size).to(device)
    c_0 = torch.randn(batch, hidden_size).to(device)
    states = [LSTMState(h_0, c_0) for _ in range(num_layers)]
    h_0 = h_0.repeat(num_layers, 1, 1)
    c_0 = c_0.repeat(num_layers, 1, 1)

    lstm = script_lstm(input_size, hidden_size, num_layers).to(device)

    lnlstm = script_lnlstm(input_size, hidden_size, num_layers, decompose_layernorm=False).to(device)

    nn_lstm = nn.LSTM(input_size, hidden_size, num_layers).to(device)

    timer = timeit.Timer("lstm(inp, states)", globals=dict(lstm=lstm, inp=inp, states=states))
    number, time_taken = timer.autorange()
    timings = timer.repeat(repeat=5, number=number)
    print(f"{min(timings):.3e} +- {np.std(timings):.3e} s")

    timer = timeit.Timer("lnlstm(inp, states)", globals=dict(lnlstm=lnlstm, inp=inp, states=states))
    number, time_taken = timer.autorange()
    timings = timer.repeat(repeat=5, number=number)
    print(f"{min(timings):.3e} +- {np.std(timings):.3e} s")

    timer = timeit.Timer("nn_lstm(inp, (h_0, c_0))", globals=dict(nn_lstm=nn_lstm, inp=inp, h_0=h_0, c_0=c_0))
    number, time_taken = timer.autorange()
    timings = timer.repeat(repeat=5, number=number)
    print(f"{min(timings):.3e} +- {np.std(timings):.3e} s")
