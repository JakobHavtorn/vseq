...
class HMLM(BaseModel):
    def __init__(
        self, num_embeddings: int, embedding_dim: int, sizes: Union[int, List[int]], num_layers: Optional[int] = None
    ):
        super().__init__()
        ...
        self.hmlstm = HMLSTM(input_size=embedding_dim, sizes=self.sizes, num_layers=num_layers)
    ...
    def forward(
        ...
        clock_rate = [(ze.squeeze() * seq_mask).sum() / x_sl.sum() for ze in z]
