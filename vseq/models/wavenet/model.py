...
Output = namedtuple("Output", ["loss", "likelihood"])
...
class WaveNet(BaseModule):
    ...
    def compute_loss(self, x: torch.FloatTensor, x_sl: list[int], output: torch.FloatTensor):
        ...
        target = x.squeeze(1)  # TODO Only works for one input channel
        ...
        target = target.floor().to(torch.int64)  # To integer

        loss = self.nll_criterion(output, target)

        # Mask and reduce likelihood
        x_sl = torch.LongTensor(x_sl) if loss.device == torch.device('cpu') else torch.cuda.LongTensor(x_sl)
        x_sl -= self.receptive_field
        mask = sequence_mask(x_sl, device=loss.device)
        loss *= mask
    ...
    def forward(self, x, x_sl):
        ...
        loss = self.compute_loss(x, x_sl, output)

        output = Output(loss=loss, likelihood=-loss)
