...
from vseq.modules.distributions import CategoricalDense
...
class WaveNet(BaseModel):
    def __init__(
        ...
        Args:
            ...
            num_bins (int): Number of classes for output (i.e. number of quantized values in target audio values)
        ...
        self.out_transform = nn.Sequential(
            nn.ReLU(),
            nn.Linear(res_channels, res_channels),
            nn.ReLU(),
        )
    ...
    #     target = target * (self.num_bins - 1)  # Transform [0, 1] to [0, 255]
    ...
    def forward(self, x: TensorType["B", "T", "C", float], x_sl: TensorType["B", int], target: TensorType["B", "T"] = None):
