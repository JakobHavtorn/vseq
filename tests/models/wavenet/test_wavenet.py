...
def test_wavenet_output_size(wavenet):
    x = generate_dummy(wavenet.receptive_field * 2 + 2)
    ...
    x = generate_dummy(wavenet.receptive_field * 4)
    ...
    assert output.shape == torch.Size([1, wavenet.receptive_field, 2])


def test_wavenet_fail_with_short_input(wavenet):
    x = generate_dummy(wavenet.receptive_field * 2)
