...
def test_script_lstm_layer_norm_script_layer(seq_len, batch, input_size, hidden_size, num_layers):
    inp = torch.randn(seq_len, batch, input_size)
    state = [LSTMState(torch.randn(batch, hidden_size), torch.randn(batch, hidden_size)) for _ in range(num_layers)]
...
def test_script_lstm_runtime_forward(seq_len, batch, input_size, hidden_size, num_layers, device):
    ...
    print(f"Native is {(timings_script.min - timings_native.min) / timings_native.min * 100:.2f}% faster")
