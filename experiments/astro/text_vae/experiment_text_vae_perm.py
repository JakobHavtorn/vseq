...
keys = ENGLISH_STANDARD + [UNKNOWN_TOKEN]
values = deepcopy(keys[:-3]) # we keep space, apostophe and UNK-token
shuffle(values)
values += keys[-3:]

map_xy = dict(zip(keys, values))
map_yx = dict(zip(values, keys))

def permute_string(s):
    s = s.lower().strip()
    s = ''.join([map_xy[c] for c in s])
    return s
    
def reverse_string(s):
    s = ''.join([map_yx[c] for c in s])
    return s
...
text_cleaner = TextCleaner(permute_string)
...
for k, v in model.state_dict(keep_vars=True).items():
    if k in prior_state_dict:
        v.requires_grad_(False)
parameters = [p for p in model.parameters() if p.requires_grad == True]
assert len(parameters) + len(prior_state_dict) == len(model.state_dict())

optimizer = getattr(torch.optim, args.optimizer)
optimizer = optimizer(parameters, lr=args.lr_max, **args.optimizer_kwargs)
...
for epoch in tracker.epochs(args.epochs):
    ...
    samples = [reverse_string(s) for s in samples]
    print(f"\nPrior sample:\n{samples[0]}\n")
