...
parser.add_argument("--seconds_pr_batch", default=240, type=int, help="batch size")
...
token_map = TokenMap(tokens=ENGLISH_STANDARD, add_delimit=True)
...
for epoch in tracker.epochs(args.epochs):
    ...
    for ((x, x_sl), (y, y_sl)), metadata in tracker.steps(train_loader):
        ...
        x = x.to(device)
        y = y.to(device)
        loss, metrics, outputs = model(x, x_sl, y, y_sl, teacher_forcing_frq=0.9)

        optimizer.zero_grad()
        loss.backward()
        nn.utils.clip_grad_value_(model.parameters(), 3.0)
        optimizer.step()
    ...
    #         x = x.to(device)
    #         y = y.to(device)
    #         loss, metrics, outputs = model(x, x_sl, y, y_sl, max_len=625)
