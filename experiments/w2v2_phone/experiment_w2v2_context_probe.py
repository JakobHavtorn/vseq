...
from vseq.models import MultiProbe, LinearProbe
...
model = MultiProbe(
    ...
    probe_args=dict(input_size=w2v2.encoder.embedding_dim, output_size=output_size)
)
model.to(device)
...
def shift_labels(features, y):
    s = args.label_shift
    y = y[s:] if s > 0 else y[:s]
    features = [f[:-s] if s > 0 else f[-s:] for f in features]
    return features, y
