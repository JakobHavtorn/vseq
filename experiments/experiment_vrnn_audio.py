...
from vseq.data.transforms import Compose, MuLawDecode, MuLawEncode, StackWaveform
...
parser.add_argument("--num_bits", default=8, type=int, help="number of quantization bins (256, 8 bits by default)")
...
for epoch in tracker.epochs(args.epochs):
    ...
    with torch.no_grad():
        ...
        if epoch % 25 == 0:
            ...
            reconstructions = [wandb.Audio(reconstructions[i].flatten().cpu().numpy(), caption=f"Reconstruction {i}", sample_rate=16000) for i in range(2)]
