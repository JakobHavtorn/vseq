...
parser.add_argument("--batch_size", default=8, type=int, help="number of mel filterbanks")
...
parser.add_argument("--label_shift", default=4, type=int, help="negative = predict to the left, positive = predict to the right")
...
def get_w2v2_features(x, x_sl):
    with torch.no_grad():
        B = x.size(0)
        T = int((x_sl.max() - 400) / 320 + 1)
        D = w2v2.encoder.embedding_dim
        features = [torch.zeros(B, T, D, device=device) for j in range(len(w2v2.encoder.layers))]
        for i in range(x.size(0)):
            T_i = int((x_sl[i] - 400) / 320 + 1)
            result = w2v2.forward(x[i:i+1, :x_sl[i].item()], padding_mask=None, mask=False, features_only=True, layer=100)
            for k, l in enumerate(result["layer_results"]):
                features[k][i, :T_i] += l[0][:, 0]
            del result
    return features
...
for epoch in tracker.epochs(args.epochs):
    ...
    for ((x, x_sl), ((y, m), y_sl)), metadata in tracker.steps(train_loader):
        ...
        features = get_w2v2_features(x, x_sl)
        features_sl = torch.floor_divide(x_sl - 400, 320) + 1
    ...
    #         features = get_w2v2_features(x)
            
    #         if args.label_shift != 0:
    #             features, y = shift_labels(features, y)
    #             if y.size(0) == 0:
    #                 tracker.update([])
    #                 continue
